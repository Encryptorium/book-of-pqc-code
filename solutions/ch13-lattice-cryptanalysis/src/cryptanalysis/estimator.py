...
def estimate_parameter_set(
    ...
    quantum bit cost.
    """
    assert k >= 1, f"estimate_parameter_set: k must be >= 1, got {k}"
    assert n >= 1, f"estimate_parameter_set: n must be >= 1, got {n}"
    assert q >= 2, f"estimate_parameter_set: q must be >= 2, got {q}"
    assert eta_1 >= 1, f"estimate_parameter_set: eta_1 must be >= 1, got {eta_1}"

    zeta = math.sqrt(eta_1 / 2.0)
    beta, m_opt = primal_beta(k, n, q, zeta)
    d = m_opt + k * n + 1
