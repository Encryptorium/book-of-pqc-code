...
def analyze_chain(chain: Iterable[CertRef]) -> ChainReport:
    ...
    """
    certs = tuple(chain)
    classes = tuple(classify_oid(c.sig_oid) for c in certs)
    unique = set(classes)
    if not classes:
        overall = "empty"
    elif "unknown" in unique:
        overall = "unknown-oid-present"
    elif unique == {"classical"}:
        overall = "classical-only"
    elif unique == {"single-pq"}:
        overall = "single-pq-only"
    elif unique == {"composite"}:
        overall = "composite-only"
    else:
        leaf = classes[0]
        if leaf in {"composite", "single-pq"} and "classical" in unique:
            overall = "mixed-classical-above-pq-leaf"
        else:
            overall = "mixed-transition"
    return ChainReport(depth=len(certs), per_cert=classes, overall=overall)
