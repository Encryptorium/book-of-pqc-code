...
def PRF(eta: int, seed: bytes, nonce: int) -> bytes:
    ...
    values.
    """
    assert eta in (2, 3), f"PRF: eta must be 2 or 3, got {eta}"
    assert len(seed) == 32, f"PRF: seed must be 32 bytes, got {len(seed)}"
    assert 0 <= nonce <= 255, f"PRF: nonce must be a byte, got {nonce}"
