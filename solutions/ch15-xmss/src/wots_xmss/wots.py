...
def checksum(msg_digits: list[int], w: int) -> list[int]:
    ...
    ell_2 = math.ceil((math.floor(math.log2(max_checksum)) + 1) / lg_w)
    # Left-shift c so that the ell_2 digits occupy the most significant
    # positions of the byte encoding, matching RFC 8391 Section 3.1.5
    # (Algorithm 5); Section 2.6 defines base_w and not this shift.
...
def wots_sign_no_checksum(
    ...
    *pk_seed* is the public chain-domain seed.
    """
    ell_1, _, _ = _ell_params(n, w)
    digest = _sha256(message)
    msg_digits = base_w(digest, w, ell_1)
    return [chain(sk[i], 0, msg_digits[i], pk_seed, i) for i in range(ell_1)]
...
def wots_verify_no_checksum(
    ...
    *pk_seed* is the public chain-domain seed.
    ...
    for i in range(ell_1):
        if chain(sig[i], msg_digits[i], w - 1 - msg_digits[i], pk_seed, i) != pk[i]:
            return False
    return True
