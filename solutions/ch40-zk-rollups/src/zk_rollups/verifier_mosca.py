...
def cadence_options(X: int, Y: int, Z: int) -> Dict[str, CadenceOption]:
    ...
    per_rollup_cycle: CadenceOption = {
        "name": "per-rollup-cycle",
        "feasible": True,
        "rotation_interval_years": 0,
        "operational_cost": "prohibitive",
        "rationale": (
            "rotate every rollup cycle; the multisig propose-audit-"
            "approve-activate-transition choreography makes this "
            "operationally prohibitive in practice"
        ),
    }

    every_n_cycles: CadenceOption = {
        "name": "every-N-rollup-cycles",
        "feasible": safe_window >= 1 and breach > 0,
        "rotation_interval_years": safe_window,
        "operational_cost": "medium",
        "rationale": (
            "rotate every N rollup cycles with N tuned so effective "
            "verifier-bytecode reuse plus migration clears the Mosca "
            "window"
        ),
    }
    ...
    governance_trigger: CadenceOption = {
        ...
        "rationale": (
            ...
            "by the council rather than by Mosca arithmetic"
        ),
    }
    ...
    return {
        "per-rollup-cycle": per_rollup_cycle,
        "every-N-rollup-cycles": every_n_cycles,
...
def recommend_cadence(X: int, Y: int, Z: int) -> Dict[str, object]:
    ...
    if breach <= 0:
        ...
        recommendation = "every-N-rollup-cycles"
