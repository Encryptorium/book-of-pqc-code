"""Pytest conftest for the Chapter 22 test suite.

Adds the selected tree's ``ch22-isogenies/src`` to ``sys.path`` so the ``isogenies``
...
PACKAGE_SRC = REPO_ROOT / IMPL / "ch22-isogenies" / "src"
