"""Pytest conftest for the Chapter 16 test suite.

Adds the selected tree's ``ch16-fors/src`` to ``sys.path`` so the ``fors_hypertree``
package can be imported without a ``pip install -e``.  Matches the pattern
set by ``tests/ch15/conftest.py``.
"""
...
PACKAGE_SRC = REPO_ROOT / IMPL / "ch16-fors" / "src"
