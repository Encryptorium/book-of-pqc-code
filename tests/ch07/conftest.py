"""Pytest conftest for the Chapter 7 test suite.

Adds the selected tree's ``ch07-lattices/src`` to ``sys.path`` so the ``lattices``
...
PACKAGE_SRC = REPO_ROOT / IMPL / "ch07-lattices" / "src"
