...
def relative_throughput(primitive: str, baseline: str, budget: str = "btc") -> float:
    ...
    #
    # Select the per-budget throughput function once, then return the
    # candidate's count divided by the baseline's. Validate budget,
    # primitive, and baseline in three separate asserts so a typo names the
    # argument it came from, and assert the baseline count is positive
    # before dividing. A return of 0.05 means the candidate clears 5 percent
    # as many transactions per block as the baseline.
    #
    # Reference: Chapter 37, 'Cutover with a composite signature'
    #
    # Proved by:
    #   tests/ch37/test_throughput_compare.py
    raise NotImplementedError("exercise: relative_throughput")
