...
def witness_reveals_pk(primitive: str) -> bool:
    ...
    # classical-secp256k1, which the chapter models as a Taproot key-path
    # spend: the tweaked public key already sits in the output script, so
    # the spend witness carries the signature alone. The three post-quantum
    # rows are modeled as P2WPKH-style commit-then-reveal, where the output
    # script hash-commits to the public key and the witness reveals both key
    # and signature at spend.
    #
