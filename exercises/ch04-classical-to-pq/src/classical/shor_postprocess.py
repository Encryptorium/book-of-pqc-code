...
def recover_factor(n: int, a: int, r: int) -> int:
    ...
    #
    # Raise ValueError on an odd period, since the identity needs r/2 to be
    # an integer. Compute x = a^{r/2} mod n and raise again when x equals n
    # - 1, the trivial square root that yields only trivial gcds. Otherwise
    # return whichever of gcd(x-1, n) and gcd(x+1, n) lies strictly between
    # 1 and n, and raise ValueError when neither does.
    #
    # Reference: Chapter 4, 'What Shor does to this'
    #
    # Proved by:
    #   tests/ch04/test_shor_postprocess.py
    raise NotImplementedError("exercise: recover_factor")
