...
class MLKEMParams:
    ...
    def ek_len(self) -> int:
        ...
        #
        # The encryption key is ByteEncode_12 of the k-element vector t_hat
        # followed by the 32-byte matrix seed rho. Twelve bits per
        # coefficient over 256 coefficients packs to 384 bytes per
        # polynomial, so the total is 384 * k + 32: 800, 1184, and 1568
        # bytes at k = 2, 3, 4.
        #
        # Reference: Chapter 11, 'A concrete ML-KEM-768 parameter and seed derivation' (FIPS 203 Table 3)
        #
        # Proved by:
        #   tests/ch11/test_mlkem_params.py
        #   tests/ch11/test_k_pke_roundtrip.py
        raise NotImplementedError("exercise: MLKEMParams.ek_len")
    ...
    def dk_pke_len(self) -> int:
        ...
        # EXERCISE: implement this function.
        #
        # The K-PKE decryption key is ByteEncode_12 of s_hat and nothing
        # else, so 384 * k bytes with no seed appended. It is the one length
        # here that carries no 32-byte tail.
        #
        # Reference: Chapter 11, 'A concrete ML-KEM-768 parameter and seed derivation' (FIPS 203 §5.1)
        #
        # Proved by:
        #   tests/ch11/test_mlkem_params.py
        #   tests/ch11/test_k_pke_roundtrip.py
        raise NotImplementedError("exercise: MLKEMParams.dk_pke_len")
    ...
    def dk_len(self) -> int:
        ...
        # EXERCISE: implement this function.
        #
        # The ML-KEM decapsulation key is dk_PKE || ek || H(ek) || z. Add
        # the two lengths already derived, then 32 for the stored hash and
        # 32 for the implicit-rejection seed: 1632, 2400, and 3168 bytes at
        # k = 2, 3, 4.
        #
        # Reference: Chapter 11, 'The Fujisaki-Okamoto wrapper' (FIPS 203 §6.1)
        #
        # Proved by:
        #   tests/ch11/test_mlkem_params.py
        #   tests/ch11/test_mlkem_roundtrip.py
        raise NotImplementedError("exercise: MLKEMParams.dk_len")
    ...
    def ct_len(self) -> int:
        ...
        # EXERCISE: implement this function.
        #
        # The ciphertext is ByteEncode_{d_u} of the compressed k-element
        # vector u followed by ByteEncode_{d_v} of the compressed scalar v.
        # A compressed polynomial at width d packs 256 * d bits into 32 * d
        # bytes, so the total is 32 * (d_u * k + d_v). At ML-KEM-768 that is
        # 960 + 128 = 1088.
        #
        # Reference: Chapter 11, 'A concrete ML-KEM-768 parameter and seed derivation' (FIPS 203 §5.2)
        #
        # Proved by:
        #   tests/ch11/test_mlkem_params.py
        #   tests/ch11/test_k_pke_roundtrip.py
        raise NotImplementedError("exercise: MLKEMParams.ct_len")
