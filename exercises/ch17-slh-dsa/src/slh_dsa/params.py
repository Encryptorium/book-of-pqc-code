...
class SLHDSAParams:
    ...
    def sig_bytes(self) -> int:
        ...
        #
        # Three components summed. R is n bytes. SIG_FORS is k * (1 + a) *
        # n, because each of the k FORS trees contributes one revealed leaf
        # value and a authentication nodes. SIG_HT is d * (ell + hp) * n,
        # because each of the d hypertree layers contributes a WOTS+
        # signature of ell chain values and an authentication path of hp
        # nodes. At SLH-DSA-SHA2-128s that is 16 + 2,912 + 4,928 = 7,856
        # bytes. The 's' sets keep this small with deep subtrees and few
        # layers; the 'f' sets trade it away for shallow subtrees that are
        # cheaper to build.
        #
        # Reference: Chapter 17, 'FIPS 205 parameter sets and signature sizes'
        #
        # Proved by:
        #   tests/ch17/test_slhdsa_roundtrip.py
        #   tests/ch17/test_vectors.py
        raise NotImplementedError("exercise: SLHDSAParams.sig_bytes")
