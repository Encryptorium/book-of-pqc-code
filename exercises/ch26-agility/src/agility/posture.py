...
def needs_rehash(stored_iterations: int, target_iterations: int) -> bool:
    ...
    #
    # Decide whether a stored password row was derived below the current
    # target iteration count. One comparison. The direction matters: a row
    # above the target must return False, because lowering a target must
    # never trigger a re-derivation that weakens a stored hash. Raising
    # PBKDF2 iterations cannot rewrite every row at once, since the
    # derivation needs the password and the service does not hold it, so the
    # agile pattern re-derives each row at that user's next successful
    # login.
    #
    # Reference: Chapter 26, 'Lifecycle management'
    #
    # Proved by:
    #   tests/ch26/test_posture.py
    raise NotImplementedError("exercise: needs_rehash")
