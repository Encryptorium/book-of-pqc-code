...
def build_cbom(
    ...
    *,
    ...
    #
    # Wrap the components in the CycloneDX 1.6 envelope: bomFormat
    # 'CycloneDX', specVersion from the module constant, the serial number
    # and integer version 1, a metadata block holding the timestamp and an
    # application component with the app name and version, and a components
    # array with one entry per touchpoint from _build_component. The
    # timestamp and serial number are parameters with fixed defaults so two
    # runs over the same inventory compare byte for byte; production tooling
    # passes the current time and a fresh UUID instead.
    #
    # Reference: Chapter 25, 'From source code to CBOM JSON'
    #
    # Proved by:
    #   tests/ch25/test_cbom_schema.py
    #   tests/ch25/test_cbom_generation.py
    raise NotImplementedError("exercise: build_cbom")
