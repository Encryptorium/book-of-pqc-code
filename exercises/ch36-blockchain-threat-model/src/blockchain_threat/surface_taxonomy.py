...
def classify_all(
    ...
    #
    # Walk the asset records in order and return a list of (surface, class)
    # pairs, taking the class from classify and the surface from the
    # record's 'surface' field coerced to str. Order is part of the
    # contract: the caller pairs the output back with the input list by
    # index, so do not sort or deduplicate. The chapter names this function
    # and states its contract but prints only the classify it calls, so this
    # is the one classifier function left to write.
    #
    # Reference: Chapter 36, 'The five surfaces'
    #
    # Proved by:
    #   tests/ch36/test_surface_taxonomy.py
    raise NotImplementedError("exercise: classify_all")
