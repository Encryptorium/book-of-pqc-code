...
def effective_quantum_preimage_bits(ps: ParameterSet) -> float:
    ...
    #
    # Start from the classical undefended figure, n_bits minus log2 of the
    # target population, and halve it. The order is the whole exercise.
    # Grover searching a space of 2**n for any of N marked items runs in
    # about sqrt(2**n / N) evaluations, so the exponent that gets halved is
    # the one the multi-target reduction already lowered. Halving first and
    # subtracting the full advantage afterwards understates the quantum cost
    # by log2(N) / 2 bits, which at SLH-DSA-SHA2-128s is nearly eight bits,
    # so the two orders are easy to tell apart numerically.
    #
    # Reference: Chapter 18, 'Grover's algorithm and hash preimage', combined with the multi-target reduction from 'Multi-target preimage advantage'
    #
    # Proved by:
    #   tests/ch18/test_multitarget.py
    raise NotImplementedError("exercise: effective_quantum_preimage_bits")
