...
def log2_fors_forgery_approx(q: int, k: int, t: int) -> float:
    ...
    #
    # This is the (q / t)**k form the literature quotes, in log2 and with no
    # expm1 anywhere: k * (log2(q) - log2(t)). Return -inf at q = 0 so it
    # matches the exact function's domain. Do not clamp the result at zero.
    # The point of having both forms is that this one runs past probability
    # one at q >= t and says nothing about having done so, and a clamp would
    # hide exactly the failure the chapter is pointing at.
    #
    # Reference: Chapter 18, 'Multi-target advantage, birthday bound, and quantum cost formulas', the q << t approximation displayed beside the exact coverage formula
    #
    # Proved by:
    #   tests/ch18/test_fors_reuse.py
    raise NotImplementedError("exercise: log2_fors_forgery_approx")
...
def expected_position_collisions(q: int, h: int) -> float:
    ...
    # EXERCISE: implement this function.
    #
    # Count unordered pairs of signatures, q * (q - 1) / 2, and multiply by
    # the chance a given pair lands on the same hypertree position, 2**-h.
    # Expectation is linear, so this needs no independence assumption. Use
    # exact integer arithmetic for the pair count before dividing: at the
    # FIPS lifetime limit q is 2**64 and the numerator does not survive a
    # float. The chapter quotes this as q**2 / 2**64 at h = 63, which is the
    # same figure with the pair count rounded up, so your exact form should
    # agree with it to a relative error of about 1/q.
    #
    # Reference: Chapter 18, 'FORS forgery and implementation attacks', the birthday estimate for FORS-instance collisions across the 2**h leaf positions
    #
    # Proved by:
    #   tests/ch18/test_fors_reuse.py
    raise NotImplementedError("exercise: expected_position_collisions")
