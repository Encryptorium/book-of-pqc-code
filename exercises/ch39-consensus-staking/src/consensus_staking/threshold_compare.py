...
def lookup(primitive: str, role: str) -> Dict[str, object]:
    ...
    #
    # Assert the primitive and the role separately, index MATRIX by
    # primitive then by role, and flatten the cell into a dict carrying
    # primitive, role, deployment_status, admits_t_of_n,
    # requires_combine_round, and rationale. The statuses are not a ranking
    # but a mix of standardization stage and applicability: 'production'
    # appears once, on BLS at no-threshold; 'incompatible' marks a pairing
    # that is not a research direction at all, such as FROST against any
    # lattice or hash primitive, since FROST is threshold Schnorr; and every
    # threshold-PQ cell is one of the research labels, which is the
    # chapter's point that an operator has no chain-deployable threshold
    # option at chain-tip 2026.
    #
    # Reference: Chapter 39, 'Decide between threshold and per-validator signing'
    #
    # Proved by:
    #   tests/ch39/test_threshold_compare.py
...
def production_ready_at(role: str) -> List[str]:
    ...
    # EXERCISE: implement this function.
    #
    # Assert the role is known, then filter PRIMITIVES down to those whose
    # MATRIX cell for that role has deployment_status exactly 'production'.
    # Iterate PRIMITIVES rather than the matrix so the result keeps the
    # chapter's table order. The comparison is against that one literal
    # string, not against a set of acceptable statuses: 'fips-final' means
    # the standard is published, not that anything is running on a chain,
    # which is why no-threshold returns BLS alone and threshold-PQ returns
    # nothing.
    #
    # Reference: Chapter 39, 'Decide between threshold and per-validator signing'
    #
    # Proved by:
    #   tests/ch39/test_threshold_compare.py
    raise NotImplementedError("exercise: production_ready_at")
