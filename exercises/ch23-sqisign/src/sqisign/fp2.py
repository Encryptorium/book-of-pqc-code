...
def fp2_inv(x: tuple[int, int], p: int) -> tuple[int, int]:
    ...
    #
    # Multiply through by the conjugate: (a + b*i)^{-1} = (a - b*i) / (a^2 +
    # b^2). The denominator is the field norm and lies in F_p, so invert it
    # once with pow(norm, -1, p) and scale both components. Raise
    # ZeroDivisionError when the norm is zero; for p = 3 mod 4 that happens
    # only for the zero element, because a^2 + b^2 = 0 with b nonzero would
    # make -1 a square mod p.
    #
...
def fp2_pow(x: tuple[int, int], n: int, p: int) -> tuple[int, int]:
    ...
    # EXERCISE: implement this function.
    #
    # Square-and-multiply over the bits of n from the low end: keep a result
    # starting at one and a base starting at x, fold the base into the
    # result whenever the current bit is set, square the base, and shift n
    # right. Return one for n = 0, and for negative n invert x first and use
    # the absolute value. The p-power Frobenius endomorphism is nothing but
    # this function applied to each coordinate at exponent p, so a wrong
    # loop here shows up as pi failing to fix the F_p-rational points.
    #
    # Reference: Chapter 23, 'The endomorphisms of E_0'
    #
    # Proved by:
    #   tests/ch23/test_deuring_correspondence.py
    raise NotImplementedError("exercise: fp2_pow")
