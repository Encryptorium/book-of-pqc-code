...
def quaternion_to_endo_action(coords: tuple[int, int, int, int],
    ...
    #
    # Evaluate the endomorphism a + b*iota + c*pi + d*(iota*pi) at P. Apply
    # each of the four endomorphisms to P, multiply each image by its
    # integer coefficient with scalar_mul on E_0, and sum the four results
    # with point_add. The coefficients are plain integers rather than the
    # half-integers of the full O_0 basis, because halving a single point is
    # ambiguous: this is the suborder Z<1, iota, pi, iota*pi>, which sits
    # inside End(E_0) at index 4. The curve coefficient for every scalar_mul
    # and point_add here is A0, since all of it happens on E_0.
    #
    # Reference: Chapter 23, 'Deuring's correspondence'
    #
    # Proved by:
    #   tests/ch23/test_deuring_correspondence.py
    raise NotImplementedError("exercise: quaternion_to_endo_action")
